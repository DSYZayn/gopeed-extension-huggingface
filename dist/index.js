...
var KNOWN_ENDPOINTS = new Set(['huggingface.co', 'hf-mirror.com', 'alpha.hf-mirror.com', 'www.modelscope.cn']);
var DEFAULT_REPO_ENDPOINT = 'hf-mirror.com';
function buildEndpointSet() {
  var custom = gopeed.settings.customEndpoints ? gopeed.settings.customEndpoints.split(';').map(function (e) {
    return e.trim();
  }).filter(Boolean) : [];
  if (custom.length === 0) return KNOWN_ENDPOINTS;
  return new Set([].concat(_toConsumableArray(KNOWN_ENDPOINTS), _toConsumableArray(custom)));
}
...
function parseModelInput(rawUrl) {
  if (!rawUrl.startsWith('model:')) {
    throw new Error("[HF Parser] parseModelInput called with invalid input: ".concat(rawUrl));
  }
  var content = rawUrl.slice('model:'.length).trim();
  var semicolonIdx = content.indexOf(';');
  var repoPath, endpoint;
  if (semicolonIdx !== -1) {
  ...
  return new URL("https://".concat(endpoint, "/").concat(repoPath, "/tree/main"));
}
gopeed.events.onResolve(async function (ctx) {
  gopeed.logger.debug('[HF Parser] ctx.req.url:', ctx.req.url);
  try {
    var url;
    var endpointSet = buildEndpointSet();
